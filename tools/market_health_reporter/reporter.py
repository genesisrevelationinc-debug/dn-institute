...
class MarketHealthReporter:
    ...
    def generate_report(self, network):
        ...
        if not metrics:
            return "No metrics data available for the specified network."
        report = f"Market Health Report for {network}:\n"
        for metric, data in metrics.items():
            report += f"- {metric}: {data['value']} (Trend: {data['trend']})\n"
            context = self.generate_report_with_rag(f"Explain the trend of {metric} in {network} with value {data['value']}")
            report += f"  Interpretation: {context}\n"
        return report
