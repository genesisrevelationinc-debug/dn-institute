...
class MarketHealthReporter:
    def __init__(self, api_key, base_url="https://dn.institute/market-health/api"):
        response = requests.get(f"{self.base_url}/metrics/{metric}", headers={"Authorization": f"Bearer {self.api_key}"})
    ...
    def generate_report(self, metric):
        metric_data = self.fetch_metric_data(metric)
        self.initialize_rag()
        report = self.generate_report_with_rag(metric_data)
        return f"Metric: {metric_data['name']}\nValue: {metric_data['value']}\nReport: {report}"